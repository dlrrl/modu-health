...
/* ── 데이터 ── */
const GYM_VALUES = [
  { icon: "🎯", title: "입점비 0원", desc: "제휴 등록에 별도 비용이 없습니다. 부담 없이 시작하세요." },
  { icon: "💰", title: "초기 수수료 0원", desc: "서비스 초기 단계에는 수수료 없이 운영됩니다." },
  { icon: "⭐", title: "초기 제휴점 우선 노출", desc: "먼저 합류한 헬스장이 앱 내 상단 노출 혜택을 가져갑니다." },
  { icon: "👥", title: "신규 고객 유입", desc: "광고비 없이 새로운 회원을 자연스럽게 만날 수 있습니다." },
];

const HOW_STEPS = [
  { n: "1", title: "제휴 등록", desc: "간단한 문의 후 담당자와 협의해 제휴를 완료합니다." },
  { n: "2", title: "회원 방문", desc: "모두의 헬스 회원이 앱에서 헬스장을 찾아 방문합니다." },
  { n: "3", title: "데이터 확인", desc: "방문 횟수와 정산 내역을 대시보드에서 확인하세요." },
];

const FAQS = [
  { q: "제휴 등록에 비용이 드나요?", a: "아니요. 입점비는 없습니다. 서비스 초기에는 수수료도 받지 않습니다." },
  { q: "기존 회원 운영에 영향이 있나요?", a: "기존 회원 운영과 완전히 별개로 운영됩니다. 추가 수익 창출 수단으로만 활용하시면 됩니다." },
  { q: "정산은 어떻게 되나요?", a: "월 매출의 90%를 실제 방문 비율에 따라 제휴 헬스장에 분배합니다. 투명하게 정산됩니다." },
  { q: "지금 제휴하면 어떤 혜택이 있나요?", a: "초기 제휴 헬스장은 앱 내 우선 노출 혜택을 받습니다. 서비스가 성장할수록 먼저 합류한 헬스장이 더 많은 기회를 가져갑니다." },
];
...
export default function App() {
  ...
  return (
    <div style={{ fontFamily: "'Noto Sans KR', 'Apple SD Gothic Neo', sans-serif", color: s.black, background: s.white, overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-ghost {
          ...
          font-size: 0.95rem; transition: border-color 0.2s, transform 0.15s; display: inline-block;
        }
        ...
        .tag { display: inline-flex; align-items: center; gap: 6px; background: #f0fdf4; color: #16a34a; font-size: 0.78rem; font-weight: 700; padding: 5px 14px; border-radius: 100px; }
        ...
        .zero-badge {
          ...
          background: #f0fdf4; color: #16a34a; font-size: 1.3rem; font-weight: 900;
      ...
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: navSolid ? "rgba(255,255,255,0.96)" : "transparent", backdropFilter: navSolid ? "blur(12px)" : "none", borderBottom: navSolid ? "1px solid #f0f0f0" : "none", transition: "all 0.3s" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="hide-mobile" style={{ display: "flex", gap: 32 }}>
            {[["제휴 혜택","#values"],["운영 방식","#how"],["선점 혜택","#early"],["FAQ","#faq"]].map(([l,h]) => (
      ...
      {/* ── HERO ── */}
      <section style={{ padding: "120px 24px 100px", background: `linear-gradient(160deg, ${s.greenLight} 0%, #fff 60%)`, textAlign: "center" }}>
        <div style={{ maxWidth: 760, margin: "0 auto" }}>
          ...
          <p style={{ fontSize: "1.05rem", color: s.gray1, lineHeight: 1.8, marginBottom: 12, maxWidth: 580, margin: "0 auto 12px" }}>
            회원에게는 더 자유로운 운동 경험을,<br />
            헬스장에는 광고 없이 새로운 유입 기회를 드립니다.
          </p>
          <p style={{ fontSize: "0.9rem", color: s.green, fontWeight: 700, marginBottom: 36 }}>
            지금은 초기 제휴 파트너를 모집하는 단계입니다.
          </p>
          <div className="hero-btns" style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            ...
            <a href="#notify" className="btn-ghost" style={{ fontSize: "1rem", padding: "16px 32px" }}>오픈 알림 신청하기</a>
          </div>
          {/* 신뢰 신호 */}
          <div style={{ display: "flex", gap: 28, justifyContent: "center", marginTop: 36, flexWrap: "wrap" }}>
            {["입점비 0원", "초기 수수료 0원", "초기 제휴점 우선 노출"].map(t => (
          ...
          <FadeIn>
            <div style={{ textAlign: "center", marginBottom: 56 }}>
              ...
              <h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900, lineHeight: 1.3, marginBottom: 12 }}>
                빈 시간대를<br />새로운 기회로 바꾸세요
              </h2>
              <p style={{ fontSize: "0.95rem", color: s.gray2, maxWidth: 460, margin: "0 auto" }}>
                광고비 없이, 계약 부담 없이. 초기 파트너에게는 더 많은 혜택을 드립니다.
              </p>
            </div>
          </FadeIn>
            ...
            {GYM_VALUES.map((v, i) => (
              <FadeIn key={v.title} delay={i * 0.08}>
                <div className="card">
                  ...
                  <p style={{ fontSize: "0.85rem", color: s.gray2, lineHeight: 1.65 }}>{v.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 24 }}>
            {HOW_STEPS.map((step, i) => (
              <FadeIn key={step.n} delay={i * 0.1}>
                <div style={{ background: "#fff", borderRadius: 16, padding: "32px 24px", border: "1.5px solid #efefef", position: "relative" }}>
                  ...
                  <div style={{ fontWeight: 700, fontSize: "1rem", marginBottom: 8 }}>{step.title}</div>
                  <p style={{ fontSize: "0.85rem", color: s.gray2, lineHeight: 1.65 }}>{step.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
          ...
          <FadeIn>
            <div style={{ textAlign: "center", marginBottom: 56 }}>
              ...
              <h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900, color: "#fff", lineHeight: 1.3, marginBottom: 12 }}>
                초기 파트너가<br />가장 많은 걸 가져갑니다
              </h2>
              <p style={{ fontSize: "0.95rem", color: "#888", maxWidth: 460, margin: "0 auto" }}>
                서비스 초반에 합류한 헬스장이 지역 내 노출과 유입 기회를 먼저 선점합니다.
              </p>
            </div>
          </FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 20, marginBottom: 48 }}>
            {[
              { icon: "🏆", title: "지역 내 선점 효과", desc: "같은 지역 헬스장 중 먼저 합류한 곳이 앱 내 상단에 노출됩니다." },
              { icon: "📈", title: "서비스 성장과 함께", desc: "회원이 늘수록 방문이 늘고, 정산도 함께 늘어납니다." },
              { icon: "🤝", title: "초기 조건 우대", desc: "정식 확장 전 합류 파트너에게는 별도 혜택을 협의드립니다." },
            ].map((item, i) => (
              <FadeIn key={item.title} delay={i * 0.1}>
                <div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 16, padding: "28px 24px", transition: "border-color 0.2s" }}
                  ...
                  <p style={{ fontSize: "0.85rem", color: "#777", lineHeight: 1.65 }}>{item.desc}</p>
                </div>
              </FadeIn>
            ))}
          </div>
      ...
      <section style={{ padding: "88px 24px", background: s.greenLight }}>
        <div style={{ maxWidth: 700, margin: "0 auto", textAlign: "center" }}>
          <FadeIn>
            ...
            <p style={{ fontSize: "0.95rem", color: s.gray1, lineHeight: 1.8, marginBottom: 32 }}>
              연간권 없이, 한 달씩. 집 근처든 직장 근처든<br />
              가장 가까운 제휴 헬스장을 자유롭게 이용하는 서비스를 준비하고 있습니다.
            </p>
            <div style={{ display: "inline-flex", gap: 32, background: "#fff", borderRadius: 20, padding: "24px 40px", border: "1.5px solid #dcfce7", marginBottom: 36, flexWrap: "wrap", justifyContent: "center" }}>
            ...
            <p style={{ fontSize: "0.9rem", color: s.gray2, marginBottom: 24 }}>
      ...
      <section style={{ padding: "100px 24px", background: s.green, textAlign: "center" }}>
        <div style={{ maxWidth: 640, margin: "0 auto" }}>
          <FadeIn>
            <p style={{ fontSize: "0.82rem", fontWeight: 800, letterSpacing: "0.18em", textTransform: "uppercase", color: "rgba(255,255,255,0.7)", marginBottom: 16 }}>초기 제휴 파트너 모집</p>
            <h2 style={{ fontSize: "clamp(1.8rem, 4.5vw, 2.8rem)", fontWeight: 900, color: "#fff", lineHeight: 1.25, marginBottom: 16 }}>
              지금, 먼저 자리를<br />잡아두세요
            </h2>
            <p style={{ fontSize: "1rem", color: "rgba(255,255,255,0.8)", lineHeight: 1.75, marginBottom: 40 }}>
              입점비와 초기 수수료 없이 먼저 테스트해보세요.<br />
              서비스가 성장할수록 초기 파트너가 더 많은 기회를 가져갑니다.
            </p>
            <a href="#partner-form" style={{ display: "inline-block", background: "#fff", color: s.green, fontWeight: 800, fontSize: "1rem", padding: "16px 40px", borderRadius: 14, transition: "transform 0.15s, box-shadow 0.15s", boxShadow: "0 4px 24px rgba(0,0,0,0.12)" }}
