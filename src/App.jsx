...
export default function App() {
  ...
  async function handlePartnerSubmit() {
    ...
    try {
      const { error } = await supabase
        ...
        .insert([{ name: form.name.trim(), gym: form.gym.trim(), contact: form.contact.trim(), note: form.note.trim() || null }]);
  ...
  const s = { green: "#16a34a", greenLight: "#f0fdf4", black: "#0d0d0d", gray1: "#444", gray2: "#888", gray3: "#bbb", gray4: "#f5f5f5", white: "#ffffff" };
    ...
    <div style={{ fontFamily: "'Noto Sans KR', 'Apple SD Gothic Neo', sans-serif", color: s.black, background: s.white, overflowX: "hidden" }}>
      <style>{`
        ...
        * { box-sizing: border-box; margin: 0; padding: 0; }
        ...
        .btn-ghost { background: transparent; color: #111; font-weight: 600; padding: 14px 28px; border-radius: 12px; border: 1.5px solid #ddd; cursor: pointer; font-size: 0.95rem; transition: border-color 0.2s, transform 0.15s; display: inline-block; }
        .btn-ghost:hover { border-color: #16a34a; transform: translateY(-1px); }
        .card { background: #fff; border: 1.5px solid #f0f0f0; border-radius: 16px; padding: 24px; transition: box-shadow 0.2s, transform 0.2s; }
        .card:hover { box-shadow: 0 8px 24px rgba(0,0,0,0.07); transform: translateY(-3px); }
        .tag { display: inline-flex; align-items: center; gap: 6px; background: #f0fdf4; color: #16a34a; font-size: 0.78rem; font-weight: 700; padding: 5px 12px; border-radius: 100px; }
        .section-label { font-size: 0.72rem; font-weight: 800; letter-spacing: 0.18em; text-transform: uppercase; color: #16a34a; display: block; margin-bottom: 10px; }
        ...
        .partner-cta-glow::after { content: ''; position: absolute; inset: -6px; border-radius: 18px; background: linear-gradient(135deg, #16a34a33, #86efac22); z-index: -1; filter: blur(12px); }
        @media (max-width: 768px) { .hide-mobile { display: none !important; } .grid-2col { grid-template-columns: 1fr !important; } }
      `}</style>

      {/* NAV */}
      <nav style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: navSolid ? "rgba(255,255,255,0.95)" : "transparent", backdropFilter: navSolid ? "blur(12px)" : "none", borderBottom: navSolid ? "1px solid #f0f0f0" : "none", transition: "all 0.3s" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", padding: "0 24px", height: 60, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
          ...
          <div className="hide-mobile" style={{ display: "flex", gap: 32 }}>
            {[["서비스 소개","#how"],["혜택","#benefits"],["파트너","#partner"],["FAQ","#faq"]].map(([l,h]) => (
      ...
      {/* HERO */}
      <section style={{ padding: "120px 24px 100px", background: `linear-gradient(180deg, ${s.greenLight} 0%, #fff 100%)`, textAlign: "center" }}>
        <div style={{ maxWidth: 720, margin: "0 auto" }}>
          <div style={{ marginBottom: 28 }}><span className="tag">🚀 서울 출시 준비 중</span></div>
          <h1 style={{ fontSize: "clamp(2rem, 5.5vw, 3.5rem)", fontWeight: 900, lineHeight: 1.2, marginBottom: 20 }}>
            헬스장, 이제<br /><span style={{ color: s.green }}>월 5만원</span>으로<br />서울 어디서든
          </h1>
          ...
          <p style={{ fontSize: "1rem", color: s.gray1, lineHeight: 1.75, marginBottom: 36 }}>연간권 없이, 한 달씩 구독하고<br />서울 제휴 헬스장을 자유롭게 다니세요.</p>
          <div style={{ display: "flex", gap: 12, justifyContent: "center", flexWrap: "wrap" }}>
            <a href="#partner-form" className="btn-primary partner-cta-wrap partner-cta-glow" style={{ fontSize: "1rem", padding: "16px 32px" }}>🏋️ 헬스장 제휴 문의하기</a>
            <a href="#notify" className="btn-ghost" style={{ fontSize: "1rem", padding: "16px 32px" }}>출시 알림 받기</a>
          </div>
        ...
        <div style={{ maxWidth: 880, margin: "60px auto 0", background: "#fff", borderRadius: 20, border: "1.5px solid #e8f5e9", overflow: "hidden", boxShadow: "0 16px 60px rgba(22,163,74,0.08)" }}>
          <div style={{ background: "#f8faf8", borderBottom: "1px solid #eee", padding: "10px 16px", display: "flex", alignItems: "center", gap: 8 }}>
            <div style={{ display: "flex", gap: 5 }}>{["#ff6b6b","#ffd93d","#6bcb77"].map(c => <div key={c} style={{ width: 10, height: 10, borderRadius: "50%", background: c }} />)}</div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", background: "#f9fdf9" }}>
            ...
            <div style={{ padding: 20 }}>
              ...
              {["강남 피트니스","마포 짐","홍대 스포츠"].map(g => <div key={g} style={{ fontSize: "0.75rem", fontWeight: 600, color: s.gray1, marginTop: 4 }}>· {g}</div>)}
            </div>
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "80px 24px", background: s.white }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <FadeIn><div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900, lineHeight: 1.3 }}>헬스장 등록,<br />이런 경험 없으셨나요?</h2>
          </div></FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16 }}>
            {PROBLEMS.map((p, i) => <FadeIn key={p.text} delay={i*0.08}><div className="card" style={{ textAlign: "center", padding: "28px 20px" }}><div style={{ fontSize: "2rem", marginBottom: 12 }}>{p.icon}</div><p style={{ fontSize: "0.875rem", color: s.gray1, fontWeight: 500, lineHeight: 1.5 }}>{p.text}</p></div></FadeIn>)}
          </div>
        </div>
      </section>
      ...
      <section style={{ padding: "80px 24px", background: s.greenLight }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <FadeIn><div style={{ textAlign: "center", marginBottom: 48 }}>
            ...
            <h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900, lineHeight: 1.3 }}>모두의 헬스가<br />다르게 만들었습니다</h2>
          </div></FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(260px, 1fr))", gap: 20 }}>
            {[{icon:"🔁",title:"월 단위 구독",body:"50,000원으로 시작, 언제든 해지 가능. 장기 계약 없습니다."},{icon:"🗺️",title:"서울 제휴 헬스장",body:"앱에서 가까운 제휴 헬스장을 찾아 QR 하나로 바로 입장."},{icon:"📊",title:"방문 기반 정산",body:"이용자가 방문한 만큼 헬스장에 정산됩니다. 투명하게."}].map((item,i) => (
              <FadeIn key={item.title} delay={i*0.1}><div className="card" style={{ background: "#fff" }}><div style={{ fontSize: "1.8rem", marginBottom: 14 }}>{item.icon}</div><div style={{ fontWeight: 700, fontSize: "1rem", marginBottom: 8 }}>{item.title}</div><p style={{ fontSize: "0.875rem", color: s.gray1, lineHeight: 1.65 }}>{item.body}</p></div></FadeIn>
            ))}
          </div>
        </div>
      </section>
      ...
      <section id="how" style={{ padding: "80px 24px", background: s.white }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <FadeIn><div style={{ textAlign: "center", marginBottom: 48 }}><span className="section-label">이용 방법</span><h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900 }}>4단계로 끝납니다</h2></div></FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 8 }}>
            {STEPS.map((step,i) => <FadeIn key={step.n} delay={i*0.1}><div style={{ textAlign: "center", padding: "24px 16px" }}><div style={{ width: 44, height: 44, borderRadius: 12, background: s.green, color: "#fff", fontWeight: 900, fontSize: "1.1rem", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 14px" }}>{step.n}</div><div style={{ fontWeight: 700, fontSize: "0.95rem", marginBottom: 6 }}>{step.title}</div><p style={{ fontSize: "0.82rem", color: s.gray2, lineHeight: 1.5 }}>{step.desc}</p></div></FadeIn>)}
          </div>
        </div>
      </section>
      ...
      <section id="benefits" style={{ padding: "80px 24px", background: s.gray4 }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          <FadeIn><div style={{ textAlign: "center", marginBottom: 48 }}><span className="section-label">이용자 혜택</span><h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900 }}>이용자에게 좋은 이유</h2></div></FadeIn>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16 }}>
            {USER_BENEFITS.map((b,i) => <FadeIn key={b.title} delay={i*0.08}><div className="card" style={{ textAlign: "center", padding: "28px 20px" }}><div style={{ fontSize: "2rem", marginBottom: 12 }}>{b.icon}</div><div style={{ fontWeight: 800, fontSize: "1rem", marginBottom: 6 }}>{b.title}</div><p style={{ fontSize: "0.82rem", color: s.gray2 }}>{b.desc}</p></div></FadeIn>)}
          </div>
        </div>
      </section>
      ...
      <section id="partner" style={{ padding: "80px 24px", background: s.black }}>
        <div style={{ maxWidth: 860, margin: "0 auto" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(180px, 1fr))", gap: 16, marginBottom: 40 }}>
            {GYM_BENEFITS.map((b,i) => <FadeIn key={b.title} delay={i*0.08}><div style={{ background: "#1a1a1a", border: "1px solid #2a2a2a", borderRadius: 16, padding: "24px 20px", textAlign: "center", transition: "border-color 0.2s" }} onMouseEnter={e=>e.currentTarget.style.borderColor="#16a34a"} onMouseLeave={e=>e.currentTarget.style.borderColor="#2a2a2a"}><div style={{ fontSize: b.icon==="0원"?"1rem":"2rem", fontWeight: b.icon==="0원"?900:400, color: b.icon==="0원"?"#4ade80":"auto", marginBottom: 12 }}>{b.icon}</div><div style={{ fontWeight: 700, fontSize: "0.95rem", color: "#fff", marginBottom: 6 }}>{b.title}</div><p style={{ fontSize: "0.82rem", color: "#777", lineHeight: 1.5 }}>{b.desc}</p></div></FadeIn>)}
          </div>
          <FadeIn><div style={{ textAlign: "center" }}><a href="#partner-form" className="btn-primary" style={{ fontSize: "1rem", padding: "16px 36px" }}>제휴 문의하기 →</a></div></FadeIn>
        </div>
      </section>
      ...
      <section id="faq" style={{ padding: "80px 24px", background: s.white }}>
        <div style={{ maxWidth: 620, margin: "0 auto" }}>
          <FadeIn><div style={{ textAlign: "center", marginBottom: 48 }}><span className="section-label">FAQ</span><h2 style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 900 }}>자주 묻는 질문</h2></div></FadeIn>
          ...
          {formStatus !== "success" ? (
            <FadeIn delay={0.1}>
              <div style={{ background: "#fff", borderRadius: 20, padding: 36, border: "1.5px solid #dcfce7" }}>
                ...
                {formStatus==="error" && <div style={{ marginTop: 12, padding: "11px 16px", background: "#fff5f5", border: "1.5px solid #fecaca", borderRadius: 10, fontSize: "0.875rem", color: "#dc2626" }}>⚠️ {formError}</div>}
                <p style={{ textAlign: "center", fontSize: "0.75rem", color: s.gray3, marginTop: 12 }}>입력하신 정보는 제휴 문의 목적으로만 사용됩니다.</p>
              </div>
            </FadeIn>
          ) : (
            <FadeIn><div style={{ background: "#fff", borderRadius: 20, padding: "48px 36px", textAlign: "center", border: "1.5px solid #dcfce7" }}>
            ...
            {notifyStatus !== "success" ? (
              <>
                <div style={{ display: "flex", gap: 8, maxWidth: 420, margin: "0 auto" }}>
                  <input type="email" value={email} onChange={e=>setEmail(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleNotifySubmit()} placeholder="이메일 주소" disabled={notifyStatus==="loading"} style={{ flex: 1, border: "1.5px solid #e0e0e0", borderRadius: 10, padding: "13px 16px", fontSize: "0.9rem" }} />
                  <button onClick={handleNotifySubmit} disabled={notifyStatus==="loading"} className="btn-primary" style={{ padding: "13px 20px", borderRadius: 10, whiteSpace: "nowrap" }}>
                    {notifyStatus==="loading" ? "..." : "알림 받기"}
                  </button>
                </div>
      ...
      <footer style={{ background: "#0d0d0d", padding: "40px 24px" }}>
        <div style={{ maxWidth: 1080, margin: "0 auto", display: "flex", flexWrap: "wrap", alignItems: "center", justifyContent: "space-between", gap: 16 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
            ...
            <div><div style={{ fontWeight: 800, color: "#fff", fontSize: "0.9rem" }}>모두의 헬스</div><div style={{ fontSize: "0.72rem", color: "#555" }}>서울 출시 준비 중</div></div>
          </div>
